...
/***/ function(module, exports, __webpack_require__) {
	...
	Features
	 ...
	 - Reply messages (usefull to return data or to let the sender know that a message is processed)
	 - Promises - when sending a message a promise will be returned
	 - Auto serialize messages
	 - Message scopes
	...
	var CrossMessenger = (_temp = _class = function (_EventEmitter) {
	    ...
	    function CrossMessenger(config) {
	        ...
	        _this._send = function (message) {
	            ...
	            if (expectReply) {
	                result = new Promise(function (resolve) {
	                    _this._waitingForReplyList[id] = resolve;
	                });
	            } else {
	            ...
	            message.id = id;
	            message.messageScope = _this._messageScope;
	            message.expectReply = expectReply;
	
	            try {
	                serializedMessage = JSON.stringify(message);
	                _this.emit('send', message);
	                _this._targetFrame.postMessage(serializedMessage, _this._targetDomain);
	            } catch (error) {
	        ...
	        _this._setHandshakeSuccess = function () {
	            if (!_this._hasHandshake) {
	                _this._hasHandshake = true;
	                _this.emit('handshake', _this);
	                _this._setReadyWhenReady();
	            }
	        };
	
	        _this._setDomSuccess = function () {
	            if (!_this._isDomReady) {
	                _this._isDomReady = true;
	
	                _this.emit('domready', _this);
	                _this._setReadyWhenReady();
	
	                if (!_this._hasHandshake) {
	                    _this.send({ name: '_handshake' }, false, true);
	                }
	            }
	        };
	
	        _this._handleReceive = function (postMessage) {
	            var message = void 0;
	            ...
	            var isValidMessage = (0, _get3.default)(message, 'messageScope') === _this._messageScope && (0, _has3.default)(message, 'id'),
	                isHandshake = isValidMessage ? (0, _get3.default)(message, 'name') === '_handshake' : null,
	                messageId = isValidMessage ? (0, _get3.default)(message, 'id') : null,
	                expectReply = (0, _get3.default)(message, 'expectReply') === true,
	                isReply = (0, _has3.default)(message, 'replyId');
	            ...
	            if (isValidMessage && isHandshake && messageId) {
	                if (!isReply) {
	                    _this.reply(message, true);
	                }
	
	                setTimeout(_this._setHandshakeSuccess.bind(_this));
	            }
	            ...
	            var name = (0, _get3.default)(message, 'name'),
	                isValidReply = (0, _has3.default)(_this._waitingForReplyList, (0, _get3.default)(message, 'replyId')),
	                replyResolver = isValidReply ? _this._waitingForReplyList[message.replyId] : null;
	
	            _this.emit('receive', message, _this);
	
	            if (isValidReply) {
	                replyResolver(message);
	                delete _this._waitingForReplyList[message.replyId];
	                _this.emit('reply', message, _this);
	            } else {
	                if (expectReply) {
	                    ...
	                    if (_this._actionRouter) {
	                        try {
	                            replyPayload = _this._actionRouter.handleMessage(message, _this);
	                        } catch (error) {
	                    ...
	                    _this.reply(_extends({}, message, {
	                        payload: replyPayload,
	                        success: success,
	                        error: error
	                    }));
	                } else if (_this._actionRouter) {
	                    _this._actionRouter.handleMessage(message, _this);
	                }
	
	                _this.emit('message', message, _this);
	            }
	        };
	
	        _this._validateConfigAndThrowOnError(config);
	
	        // configureable
	        _this._targetFrame = config.targetFrame && 'contentWindow' in config.targetFrame ? config.targetFrame.contentWindow : config.targetFrame;
	        _this._targetDomain = config.targetDomain;
	        _this._messageScope = config.messageScope;
	        _this._actionRouter = (0, _get3.default)(config, 'actionRouter');
	    ...
	    _createClass(CrossMessenger, [{
	        ...
	        value: function _validateConfigAndThrowOnError(config) {
	            var required = ['targetFrame', 'targetDomain', 'messageScope'];
	
	            required.forEach(function (property) {
	                if (!(0, _has3.default)(config, property)) {
	...
	}(_eventemitter2.default), _class._idCounter = 0, _class._registeredMessageScopes = [], _temp);
