...
class CrossMessenger extends EventEmitter {
    ...
    constructor({targetFrame, targetOrigin = '*', messageScope, messageHandler } = config) {
        ...
        // protected
        this._waitingForReplyList = {};                                             // object indexed by messageId's an with array as value where the first element is the resolver and the second the rejector ( { 73: [resolve, reject] })
    ...
    _send = (message, expectReply = false) => {
        ...
        if (expectReply) {
            result = new Promise((resolve, reject) => {
                this._waitingForReplyList[id] = [resolve, reject];
            });
        } else {
    ...
    _handleReceive = (postMessage) => {
        ...
        const isValidReply = replyId in this._waitingForReplyList,
            replyResolver = isValidReply ? this._waitingForReplyList[replyId][0] : null,
            replyRejector = isValidReply ? this._waitingForReplyList[replyId][1] : null;
        ...
        if (isValidReply) {
            ...
            if (expectReply) {
                ...
                if (this._messageHandler) {
                    const replyer = (success, ...args) => this.reply({
                         ...message,
                         payload: args,
                         success,
                         error
                    ...
                    try {
                      new Promise((resolve, reject) => {
                          this._messageHandler(message, resolve, reject, this);
                       }).then(replyer.bind(this, true)).catch(replyer.bind(this, false)); 
                    } catch(catchedError) {
                        error = String(catchedError)
                        success = false;
                     
                        replyer(success, error);
                    }
                }
